...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not position.is_hive:
            ...
            position, distance = position.entrance, distance + 1
        return None
...
# The WallAnt class
class WallAnt(Ant):
    """Serves as a barrier between the Ants and the Bees
    
    Has a large amount of health but deals no damage to enemy Bees"""
    name = 'Wall'
    damage = 0
    food_cost = 4
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        left_place = self.place.exit
        while left_place != None:
            if left_place.ant != None:  
                if not left_place.ant.buffed:
                    left_place.ant.buff()
                if left_place.ant.is_container == True and left_place.ant.ant_contained != None and not left_place.ant.ant_contained.buffed:
                    left_place.ant.ant_contained.buff()
            left_place = left_place.exit
        super().action(gamestate)
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared > 0:
            if not (self.slowed > 0 and gamestate.time%2 == 1):
                self.scared -= 1
